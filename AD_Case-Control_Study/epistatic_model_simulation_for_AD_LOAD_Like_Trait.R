...
create.simluated.data.LOAD.like.trait <- function(){
  ...
  vector.of.epi.QTN <-c(vector.of.add.QTN[1], sample(2:ncol(genotypes.for.n.2099.5000.marker.subset)[-vector.of.add.QTN[1]], 1, replace = FALSE))
  ...
  if(Additive.QTN.number > 1){
    for(i in 2:Additive.QTN.number) additive.component <- additive.component + (additive.effect.trait.object[,i]*(additive.effect^i))
  }else{
...
create.simluated.data.LOAD.like.trait()
