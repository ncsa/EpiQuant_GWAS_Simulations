...
create.simluated.data <- function(){
  ...
  vector.of.add.QTN <- sample(1:nrow(genotypes.non.monomorphic.5K.subset), Additive.QTN.number, replace = FALSE)
  Add.QTN.genotypic.information <- genotypes.non.monomorphic.5K.subset[vector.of.add.QTN,]
  ...
  vector.of.epi.QTN <- c(vector.of.add.QTN[1],sample(1:nrow(genotypes.non.monomorphic.5K.subset), 1, replace = FALSE))
  Epi.QTN.genotypic.information <- genotypes.non.monomorphic.5K.subset[vector.of.epi.QTN,]
  ...
  #Set a large effect for the APOE main effect
  additive.component <- additive.component + (additive.effect.trait.object[,1]*(APOE.main.effect))
  ...
  base.line.trait.temp <- cbind(rownames(base.line.trait),base.line.trait)
  colnames(base.line.trait.temp)[1] <- "<Trait>"
  colnames(base.line.trait.temp)[2] <- "the.normal.random.variables.1"
  
  #write.table(base.line.trait.temp, "Baseline_Trait_n=2648.txt", row.names = FALSE, sep = "\t",  quote = FALSE) 
 ...
 # rownames(base.line.trait) <- as.character(base.line.trait[,3])
  ...
  for(i in heritabilities.vector){
    ...
    if(i == 0){ 
      ...
      #Take a subset of 1,000 of these individuals
      #seed.number.for.1K.individuals <- sample(-1000000:1000000, 1) #On August 24, 2017 at 4:47 pm I ran this and obtained a seed number of -167045
      seed.number.for.1K.individuals <- -167045
      set.seed(seed.number.for.1K.individuals)
      
      vector.of.row.nums.for.1K.subset <- sample(1:nrow(simulated.data), 1000, replace = FALSE)
      simulated.data.1K.subset.of.inds <- simulated.data[vector.of.row.nums.for.1K.subset,]
      
      write.table(simulated.data.1K.subset.of.inds, paste("Simulated.Data.", replicates,".Reps",
                                                          ".Herit.",i,"n=1000.txt", sep = ""), row.names = FALSE, sep = "\t",  quote = FALSE) 
      ...
      write.table(total.variance, "Total.variance.2648.inds.txt", row.names = FALSE, sep = "\t",  quote = FALSE)       
      ...
      additive.component.1K.inds <- data.frame(additive.component[vector.of.row.nums.for.1K.subset,]) 
      rownames(additive.component.1K.inds) <- rownames(additive.component)[vector.of.row.nums.for.1K.subset]                                            
      additive.genetic.variance.1K.inds <- var(additive.component.1K.inds)
      
      epistatic.component.1K.inds <- data.frame(epistatic.component[vector.of.row.nums.for.1K.subset,]) 
      rownames(epistatic.component.1K.inds) <- rownames(epistatic.component)[vector.of.row.nums.for.1K.subset]                                            
      epistatic.genetic.variance.1K.inds <- var(epistatic.component.1K.inds)      
      
      base.line.trait.1K.inds <- additive.component.1K.inds+epistatic.component.1K.inds
      
      
      base.line.trait.1K.inds.temp <- cbind(rownames(base.line.trait.1K.inds),base.line.trait.1K.inds)
      colnames(base.line.trait.1K.inds.temp)[1] <- "<Trait>"
      colnames(base.line.trait.1K.inds.temp)[2] <- "the.normal.random.variables.1"
      
      write.table(base.line.trait.1K.inds.temp, "Baseline_Trait_n=1000.txt", row.names = FALSE, sep = "\t",  quote = FALSE) 

      
      residual.variance.1K.inds <-( ((additive.genetic.variance.1K.inds+epistatic.genetic.variance.1K.inds)/i) - additive.genetic.variance.1K.inds - epistatic.genetic.variance.1K.inds)
      
      total.variance.1K.inds <- additive.genetic.variance.1K.inds+epistatic.genetic.variance.1K.inds
      write.table(total.variance.1K.inds, "Total.variance.1000.inds.txt", row.names = FALSE, sep = "\t",  quote = FALSE) 
      
      the.seed.number.vector.1K.subset <- NULL
      col.name.vector <- NULL
      for(j in 1:replicates){
        seed.number <- sample(-1000000:1000000, 1)
        set.seed(seed.number)
        the.normal.random.variables.1K.inds <- rnorm(nrow( base.line.trait.1K.inds), mean = 0, sd = sqrt(residual.variance.1K.inds))
        the.base.line.trait.plus.normal.random.variables.1K.inds <-  base.line.trait.1K.inds+the.normal.random.variables.1K.inds 
        if(j == 1){
          simulated.data.1K.subset.of.inds <- the.base.line.trait.plus.normal.random.variables.1K.inds
        }else{
          simulated.data.1K.subset.of.inds <- cbind(simulated.data.1K.subset.of.inds, the.base.line.trait.plus.normal.random.variables.1K.inds)
          colnames(simulated.data.1K.subset.of.inds)[j] <- paste(colnames(simulated.data.1K.subset.of.inds)[j],".",j,sep = "")
        }
        the.seed.number.vector.1K.subset <- c(the.seed.number.vector.1K.subset, seed.number)
        col.name.vector <- c(col.name.vector, paste("Heritability_",i, "_Rep_", j, sep = ""))
      }#end for(j in 1:replicates)
      
      
      colnames(simulated.data.1K.subset.of.inds)  <- col.name.vector
      
      #Format the output file for the simulated phenotypes
      simulated.data.1K.subset.of.inds <- cbind(rownames(base.line.trait.1K.inds), simulated.data.1K.subset.of.inds)
      colnames(simulated.data.1K.subset.of.inds)[1] <- "<Trait>"
      
      write.table(the.seed.number.vector.1K.subset, paste("Seed.number.for.", replicates,".Reps",
                                                ".Herit.",i,"n=1000.txt", sep = ""), row.names = FALSE, col.names = FALSE, sep = "\t",  quote = FALSE)
      write.table(simulated.data.1K.subset.of.inds, paste("Simulated.Data.", replicates,".Reps",
                                        ".Herit.",i,"n=1000.txt", sep = ""), row.names = FALSE, sep = "\t",  quote = FALSE) 
     ...
     # seed.number.for.300.individuals <- -434778
      ...
      additive.component.300.inds <- data.frame(additive.component.1K.inds[vector.of.row.nums.for.300.subset,]) 
      rownames(additive.component.300.inds) <- rownames(additive.component.1K.inds)[vector.of.row.nums.for.300.subset]                                            
      additive.genetic.variance.300.inds <- var(additive.component.300.inds)
      
      epistatic.component.300.inds <- data.frame(epistatic.component.1K.inds[vector.of.row.nums.for.300.subset,]) 
      rownames(epistatic.component.300.inds) <- rownames(epistatic.component.1K.inds)[vector.of.row.nums.for.300.subset]                                            
      ...
      base.line.trait.300.inds.temp <- cbind(rownames(base.line.trait.300.inds),base.line.trait.300.inds)
      ...
      #Format the output file for the simulated phenotypes
      simulated.data.300.subset.of.inds <- cbind(rownames(base.line.trait.300.inds), simulated.data.300.subset.of.inds)
...
setwd("/Users/adminuser/Box Sync/Lipka_Mainzer_Chen_Epistasis_Shared_Folder/Simulation_Study/NCRPIS_aka_Ames/")
home.dir <- getwd()
...
load("50K.and.5K.GBS.SNPs.for.Ames.2648.Rdata")
...
heritabilities.vector <- 0.34

#Size of the additive effect of the largest QTL (must be (-1,1) but preferably (0,1)). I want
# the main effect of the first additive QTN to be substantially larger than those of
# the remaining QTN
APOE.main.effect <- 0.90
additive.effect <- 0.40


#Size of the epistatic effect of the largest QTL (must be (-1,1) but preferably (0,1)|
epistatic.effect <- 0.70
